...
def main():
    ...
    #-----------------------------------------------------------------------------------------------------------
    """
    osn_base='C:/Users/nikita/Desktop/test/database.xlsx'
    all_base='C:/Users/nikita/Desktop/test/all.xlsx'
    con_base='C:/Users/nikita/Desktop/test/contact.xlsx'
    """
    osn_base='bot_top/database.xlsx'
    all_base='bot_top/all.xlsx'
    con_base='bot_top/contact.xlsx'
    ...
    def acception(call):
        ...
        if l_tt[0] == 'accept':
            ...
            if check_in_bd(anketa,osn_base):
                bot_checker.send_message(call.message.chat.id, "Анкета уже добавлена в базу данных")
            else:
            ...
            mes=bot_checker.send_message(call.message.chat.id, "Укажите причину отказа анкете №"+l_tt[1]+"(например '1.3;Проблема' )")
    ...
    def ratata(message):
        ...
        if check_float(l[0]):
            ...
            if l[0]=="ban_on":
                ...
                bot_checker.send_document(message.chat.id, doc)
                ...
                bot_checker.send_message(message.from_user.id,"Находится в разработке")
                """
            ...
            """
                ...
                bot_checker.send_message(405027580, "Время использования биржи для анкеты "+str(l1[0])+" увеличено на "+str(l1[1])+" дней(-ь)\nАктивна до "+str(tt))
                bot_checker.send_message(741710024, "Время использования биржи для анкеты "+str(l1[0])+" увеличено на "+str(l1[1])+" дней(-ь)\nАктивна до "+str(tt))
                ...
                bot_checker.send_message(405027580, "Время использования биржи для анкеты "+str(l1[0])+" уменьшено на "+str(l1[1])+" дней(-ь)\nАктивна до "+str(tt))
                bot_checker.send_message(741710024, "Время использования биржи для анкеты "+str(l1[0])+" уменьшено на "+str(l1[1])+" дней(-ь)\nАктивна до "+str(tt))            
    ...
    def selection(anketa_pokup,database):
        ...
        for i in range(20,num):
            ...
            if str(sheet_ranges_1["N"+str(i)].value)=="1":
                #bot.send_message(get_id_po_ankete(anketa_pokup,osn_base),str(sheet_ranges_1["Q"+str(i)].value))
                #bot.send_message(get_id_po_ankete(anketa_pokup,osn_base),str(sheet_ranges_1["B"+str(i)].value))
                #bot.send_message(get_id_po_ankete(anketa_pokup,osn_base),str(column_category[i-1].value))
                #bot.send_message(get_id_po_ankete(anketa_pokup,osn_base),"________________________")
        ...
        if check_com(message)==0:    
            ...
            if test_anketa(anketa):
                if str(get_id_po_ankete(anketa,osn_base))==str(message.from_user.id):
                    ...
                    bot.send_message(message.from_user.id,"Данная анкета не пренадлежит вам",reply_markup = keyboard_start_again)
            else:
